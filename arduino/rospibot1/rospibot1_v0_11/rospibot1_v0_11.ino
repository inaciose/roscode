// rospibot1 0.11
// update: pid control (instead of direct pwm)
...
void messageCb( const geometry_msgs::Twist& msg){
  ...
  rightSpeedPidSetPoint = (speed_lin/wheel_rad) + ((speed_ang*wheel_sep)/(2.0*wheel_rad));
  leftSpeedPidSetPoint = (speed_lin/wheel_rad) - ((speed_ang*wheel_sep)/(2.0*wheel_rad));

  //rightSpeedPidSetPoint = 1 * speed_lin + speed_ang * wheel_rad / 2.0;
  //leftSpeedPidSetPoint = 1 * speed_lin + speed_ang * wheel_rad / 2.0;
}
...
void loop() {
  ...
  Motor_Left(leftMotorPwmOut);
  Motor_Right(rightMotorPwmOut);
...
void Update_PID() {
  ...
  //motion(leftMotorPwmOut, rightMotorPwmOut);
...
void do_Left_Encoder() {
  ...
  encoderLeftPulses -= LeftEncoderBSet ? -1 : +1;
}
...
void do_Right_Encoder() {
  ...
  encoderRightPulses += RightEncoderBSet ? -1 : +1;
}
...
boolean leftSpeedPidCompute() {
  ...
  if(useSteeringPid) {
    ...
    leftSpeedPidInput = abs(encoderLeftPulses) + (abs(encoderLeftPulses) * steeringPidOutput / 100);
  } else {
    leftSpeedPidInput = abs(encoderLeftPulses);
  }
...
boolean rightSpeedPidCompute() {
  ...
  if(useSteeringPid) {
    ...
    rightSpeedPidInput = abs(encoderRightPulses) - (abs(encoderRightPulses) * steeringPidOutput / 100); 
  } else {
    rightSpeedPidInput = abs(encoderRightPulses);
  }  
...
boolean steeringPidCompute() {
  ...
  steeringPidInput = abs(encoderLeftTotalPulses) - abs(encoderRightTotalPulses);
