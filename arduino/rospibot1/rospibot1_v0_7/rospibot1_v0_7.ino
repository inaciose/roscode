...
void Update_Batteries() {
  ...
  if(millis() >= battery_sense_timer) {
    battery_sense_timer = millis() + BATTERY_SENSE_DELAY;
    // do battery1
    int readValue1 = analogRead(BATTERY_SENSE_PIN1);
    readValue1 = readValue1 * (BATTERY_MAXVOLT1 / BATTERY_MAXREAD1);
    battery_volt1 = (battery_volt1 * (BATTERY_OLD_RATE) + (float)readValue1 * (BATTERY_NEW_RATE)) / 2.0;
    // do battery2
    int readValue2 = analogRead(BATTERY_SENSE_PIN2);
    readValue2 = readValue1 * (BATTERY_MAXVOLT2 / BATTERY_MAXREAD2);
    battery_volt2 = (battery_volt2 * (BATTERY_OLD_RATE) + (float)readValue2 * (BATTERY_NEW_RATE)) / 2.0;
  }
