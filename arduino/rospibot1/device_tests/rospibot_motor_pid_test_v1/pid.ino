...
boolean leftSpeedPidCompute() {
  ...
  if(useSteeringPid) {
    ...
    leftSpeedPidInput = abs(encoderLeftPulsesPID) + (abs(encoderLeftPulsesPID) * steeringPidOutput / 100);
  } else {
    leftSpeedPidInput = abs(encoderLeftPulsesPID);
  }
...
boolean rightSpeedPidCompute() {
  ...
  if(useSteeringPid) {
    ...
    rightSpeedPidInput = abs(encoderRightPulsesPID) - (abs(encoderRightPulsesPID) * steeringPidOutput / 100); 
  } else {
    rightSpeedPidInput = abs(encoderRightPulsesPID);
  }  
