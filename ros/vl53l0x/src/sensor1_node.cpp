...
int main(int argc, char **argv) {
    ...
    // ros sensor_msgs LaserRange setup
    char device_frame[] = "laser_frame";
    uint16_t num_readings = 40;
    double laser_frequency = 1;

    msg.header.frame_id = device_frame;
    msg.angle_min = -1.570759;
    msg.angle_max = 1.570759;
    msg.angle_increment = 3.14159 / num_readings;
    msg.time_increment = (float)(1.0 / laser_frequency) / (float)num_readings;
    msg.scan_time = 1; //(millis()-scan_start)/1000.0;
    msg.range_min = 0.04;
    msg.range_max = 2.0;
	...
	if(device_status == VL53L0X_ERROR_NONE) {
	    ...
	    //if(!device_read.RangeStatus) {
	        uint16_t distance = device_read.RangeMilliMeter;

		// LaserScan fake data
		uint16_t position = 0;
		while(position < num_readings) {
		    ranges[position] = (float) distance / 1000.0;
                    intensities[position] = 100 + position;
		    position++;
		}
