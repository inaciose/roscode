...
#current_milli_time = lambda: int(round(time.time() * 1000.0))
...
#Subscribers and Publishers
		...
		#new publisher
		self._Looptime = rospy.Publisher('looptime',Int16,queue_size = 10)
...
#Calculate orientation from accelerometer and gyrometer
	...
	def _HandleReceivedLine(self,  line):
		...
		if(len(line) > 0):
			...
			#rospy.logwarn(line)
                        #self._serialline = line
                        #self._Serialline.publish(self._serialline)

                        #self._ilooptime = current_milli_time() - self._ilooptimeLast
	                #self._ilooptimeLast = current_milli_time()
			#self._Ilooptime.publish(self._ilooptime)
...
#######################################################################################################################
                                ...
                                if(lineParts[0] == 'x'):
                                        self._looptime = int(lineParts[1])

                                        self._Looptime.publish(self._looptime)
